...
export const TotalsSection = ({
  ...
  const totalDeposits = operations.filter(op => op.type === "deposit").reduce((total, op) => total + op.amount, 0);
  const totalWithdrawals = operations.filter(op => op.type === "withdrawal").reduce((total, op) => total + op.amount, 0);

  // Separate transfers received and sent if clientName is provided
  const transfersReceived = clientName ? operations.filter(op => op.type === "transfer" && op.toClient === clientName).reduce((total, op) => total + op.amount, 0) : 0;
  const transfersSent = clientName ? operations.filter(op => op.type === "transfer" && op.fromClient === clientName).reduce((total, op) => total + op.amount, 0) : 0;

  // Total transfers (for backwards compatibility when clientName is not provided)
  const totalTransfers = !clientName ? operations.filter(op => op.type === "transfer").reduce((total, op) => total + op.amount, 0) : 0;

  // Calculate direct operations received and sent if clientName is provided
  const directOperationsReceived = clientName ? operations.filter(op => op.type === "direct_transfer" && op.toClient === clientName).reduce((total, op) => total + op.amount, 0) : 0;
  const directOperationsSent = clientName ? operations.filter(op => op.type === "direct_transfer" && op.fromClient === clientName).reduce((total, op) => total + op.amount, 0) : 0;

  // Total direct operations (for backwards compatibility when clientName is not provided)
  const totalDirectOperations = !clientName ? operations.filter(op => op.type === "direct_transfer").reduce((total, op) => total + op.amount, 0) : 0;
  ...
  const netMovement = clientName ? totalDeposits + transfersReceived + directOperationsReceived - totalWithdrawals - transfersSent - directOperationsSent : totalDeposits + totalTransfers + totalDirectOperations - totalWithdrawals;
  if (isMobile) {
    return <div className="mt-6 p-4 border rounded-lg bg-muted/30">
