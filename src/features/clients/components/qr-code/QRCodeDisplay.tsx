...
export const QRCodeDisplay = ({ clientId, clientName, size, onHide }: QRCodeDisplayProps) => {
  ...
  const {
    ...
    handleCopyLink,
    handleOpenLink
  } = useQRCodeGeneration({ clientId, canvasRef, size });
      ...
      <div className="flex flex-col items-center gap-4 w-full">
        ...
        <div className="flex flex-col items-center gap-3 w-full">
          ...
          <div className="flex gap-2 w-full">
            ...
            <Button variant="outline" size="sm" className={cn("flex-1 border-violet-200 hover:bg-violet-100 hover:text-violet-700 transition-all gap-2", isLoading && "animate-pulse")} onClick={generateQRAccess} disabled={isLoading}>
              <RefreshCw className={`h-3.5 w-3.5 ${isLoading ? 'animate-spin' : ''}`} />
              <span className="text-xs">Refresh</span>
            </Button>
          </div>
          
          <Button variant="ghost" size="sm" className="w-full text-violet-500 hover:text-violet-700 hover:bg-violet-100" onClick={onHide}>
            Masquer le code QR
          </Button>
        </div>
      </div>
