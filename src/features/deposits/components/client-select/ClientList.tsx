    ...
    <>
      {clients.length === 0 ? (
        ...
        clients.map(client => (
          <div
            key={client.id}
            className={`flex items-center justify-between w-full py-6 px-4 cursor-pointer touch-manipulation select-none hover:bg-accent border-b ${selectedClient === client.id.toString() ? 'bg-accent/50' : ''}`}
