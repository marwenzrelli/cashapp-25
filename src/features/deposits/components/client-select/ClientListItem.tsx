...
export const ClientListItem = ({
  ...
  return <div onClick={e => onClick(client.id.toString(), e)} onTouchEnd={e => onClick(client.id.toString(), e)} data-client-id={client.id.toString()} className={`
        rounded-lg my-0.5 mx-1.5 py-1 px-1.5 transition-all relative
        ${isSelected ? 'bg-primary/15 border-l-4 border-primary shadow-sm' : 'hover:bg-muted/50 active:bg-muted/70'}
      `} style={{
    ...
    touchAction: 'manipulation'
      ...
      {isSelected && <div className="absolute bottom-1 right-1 w-6 h-6 rounded-full bg-primary flex items-center justify-center cursor-pointer hover:bg-primary/80" onClick={handleRemove}>
